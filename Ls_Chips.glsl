...
vec3 getcol(int i) {
  ...
  uv += 0.5;
  uv /= 1024.0;
  vec3 texel = texture2D(adsk_texture_grid, uv).rgb;
  return texel;
}
...
int getch(int s, int i) {
  ...
  uv += 0.5;
  uv /= 1024.0;
  float texel = texture2D(adsk_texture_grid, uv).r;
  return int(texel);
}
...
float print(int s, vec2 where) {
  ...
  float sdf = texture2D(adsk_texture_grid, fontuv).a;
